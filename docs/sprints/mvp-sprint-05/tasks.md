...
- [x] 001 — Verify clean baseline: `pnpm typecheck && pnpm test` passes on
  `main` with no changes
  (`src/`)

### Core implementation

- [x] 002 — Create `src/agent/tools.ts`: export `WikilinkToolFound`,
...
- [x] 003 [test] — Create `src/agent/tools.test.ts`: unit tests for all five
...
- [x] 004 — Refactor `src/agent/generation-loop.ts` to export
...
- [x] 005 [test] — Update `src/__tests__/agent-vault.integration.test.ts`: add
...
- [x] 006 — Update `src/cli/index.ts` to use `GenerationSession`: replace
...
- [x] 007 [test] — Update `src/__tests__/cli.integration.test.ts`: remove the
...
- [x] 008 — Update `docs/sprints/overview.md` to mark sprint complete
